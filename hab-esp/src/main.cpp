...
SoftwareSerial sendto(U_RX, U_TX);
...
void take_picture() {
  ...
  sendto.write(IMAGE_START_MARKER);
  uint32_t img_size = fb->len;
  sendto.write((uint8_t *)&img_size, 4);
  sendto.write(fb->buf, img_size);
  sendto.write(IMAGE_END_MARKER);
...
void loop() {
  Serial.println("Attempting picture");
